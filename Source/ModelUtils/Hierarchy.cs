...
public static class Hierarchy {
  ...
  public static void MoveToParent(Transform child, Transform parent, Vector3? newPosition = null,
                                  Quaternion? newRotation = null, Vector3? newScale = null) {
  ...
  public static Transform FindTransformInChildren(Transform parent, string name, Transform defValue = null) {
  ...
  public static Transform FindTransformByPath(Transform parent, string[] path, Transform defValue = null) {
    ...
    if (obj == null && defValue != null) {
      HostedDebugLog.Warning(
          parent, "Cannot find path: {0}. Using a fallback: {1}", MakePath(path), DbgFormatter.TransformPath(defValue));
      return defValue;
    }
    return obj;
  }
  ...
  public static string[] ListHierarchy(Transform parent, string pathPrefix = "") {
    var res = new List<string>();
    GatherHierarchyNames(parent, pathPrefix, res);
    return res.ToArray();
  }
  ...
  public static string[] SplitAndUnescapePath(string escapedPath) {
    return ReSplitEscapedPath.Split(escapedPath).Select(UnescapeName).ToArray();
  }
  ...
  public static string MakePath(string[] unescapedNames) {
    return string.Join("/", unescapedNames.Select(EscapeName).ToArray());
  }
  ...
  static void GatherHierarchyNames(Transform parent, string pathPrefix, List<string> names) {
    ...
    for (var i = 0 ; i < parent.childCount; i++) {
      ...
      GatherHierarchyNames(child, fullName, names);
    }
  }
  ...
  static void FindTransformByPathInternal(
    ...
    if (pattern == "**") {
      ...
      if (reducedNames.Length == 0) {
        return;  // It's an ambiguity.
      }
      FindTransformByPathInternal(parent, reducedNames, currentLevel, ref bestMatchPathLength, ref bestMatchNode);
      for (var i = 0; i < parent.childCount; ++i) {
        FindTransformByPathInternal(
            parent.GetChild(i), names, currentLevel + 1, ref bestMatchPathLength, ref bestMatchNode);
      }
      return;
    }
    ...
    for (var i = 0; i < parent.childCount; ++i) {
      ...
      if (pattern == "*" || PatternMatch(pattern, child.name)) {
        if (index == -1) {
          FindTransformByPathInternal(
              child, reducedNames, currentLevel + 1, ref bestMatchPathLength, ref bestMatchNode);
        } else if (index-- == 0) {
          FindTransformByPathInternal(
              child, reducedNames, currentLevel + 1, ref bestMatchPathLength, ref bestMatchNode);
